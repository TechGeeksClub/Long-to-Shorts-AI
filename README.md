...
- Automatic topic segmentation and highlight selection
...
4. Adjust clip boundaries, subtitles, and non-speech cut ranges if needed.
...
## Enhanced SEO with Ollama

Ollama is optional. When **Professional LLM SEO** is disabled, the application
creates local SEO suggestions without using the internet or an external LLM.

When enabled, the application connects to Ollama at
...
- When Professional LLM SEO is enabled, clip text is sent only to the configured
  Ollama server. The default address points to the local machine.
